...
//public class CucumberReRunTest {
	...
	public static void tearDown() throws CheetahException {
		...
		if (CheetahEngine.props.getProperty("close.browser") != null
			...
			CheetahEngine.logger.logMessage(null, CucumberReRunTest.class.getName(), "Close Driver Attempt",
					Constants.LOG_INFO, false);
			// CheetahEngine.getDriverInstance().close();

			if (CheetahEngine.getDriverInstance() != null) {
				try {
					CheetahEngine.logger.logMessage(null, CucumberReRunTest.class.getName(),
							"Driver: " + CheetahEngine.getDriverInstance().toString(), Constants.LOG_INFO, false);
					CheetahEngine.logger.logMessage(null, CucumberReRunTest.class.getName(), "Quit Driver Attempt",
							Constants.LOG_INFO, false);
					CheetahEngine.getDriverInstance().quit();
				} catch (Exception e) {
					CheetahEngine.logger.logMessage(e, CucumberReRunTest.class.getName(),
